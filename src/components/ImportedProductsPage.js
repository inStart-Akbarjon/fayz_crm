...
const ImportedProductsPage = () => {
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get('http://localhost:5000/api/imported-products');
    ...
    const handleEditProduct = async (productId, updatedData) => {
        try {
            const response = await axios.put(`http://localhost:5000/api/imported-products/${productId}`, updatedData);
    ...
    const handleDeleteProduct = async (productId) => {
        try {
            await axios.delete(`http://localhost:5000/api/imported-products/${productId}`);
    ...
    const renderSummary = (data) => {
        ...
        const calculateRevenue = (product, clients) => {
            ...
            const totalClientSum = clients.reduce((total, client) => {
                ...
                const clientProduct = client.products.find(p => p.name === product.name && p.category === product.category);
        ...
        return (
                ...
                <div className="legend-item">
                    ...
                    <p>Всего метров: <span style={{ color: COLORS[4], fontWeight: 500 }}>{totalLength}м</span></p>
                </div>
                <div className="legend-item">
                    <span className="color-box" style={{ backgroundColor: COLORS[0] }}></span>
                    <p>Доступно: <span style={{ color: COLORS[0], fontWeight: 500 }}>{available}м</span></p>
                </div>
                <div className="legend-item">
                    <span className="color-box" style={{ backgroundColor: COLORS[1] }}></span>
                    <p>Продано: <span style={{ color: COLORS[1], fontWeight: 500 }}>{sold}м</span></p>
                </div>
            </div>
        );
    };
...
<select name="country" style={{marginLeft: `3%`}}  className='input' value={filters.country} onChange={handleFilterChange}>
...
<select name="width" style={{marginLeft: `3%`}}  className='input' value={filters.width} onChange={handleFilterChange}>
...
<select name="surface" style={{marginLeft: `3%`}}  className='input' value={filters.surface} onChange={handleFilterChange}>
...
<select name="color" style={{marginLeft: `3%`}}  className='input' value={filters.color} onChange={handleFilterChange}>
        ...
        <option key={color} value={color}>{color}</option>
            ...
            {filteredProducts.map((product) => (
                        <div key={product._id} className={`product-item`}>
                            ...
                            <div key={product._id} className='border' style={{ backgroundColor: `var(${product.color})`, color: `var(${product.color})`, borderRadius: 5, alignItems: `center`}} >......|</div>
                            <div>{product.category}</div>|
                            <div>{product.price}с.</div>|
                            <div>{product.sum}с.</div>|
                            <div>{product.length}м</div>|
                            <div>{product.thickness}</div>|
                            <div>{formatDate(product.creationDate)}</div>
                            <div style={{}}>
                                <button className='info' style={{fontWeight: 600}} onClick={() => handleProductClick(product)}>Инфо</button>
                                <button className='info' style={{fontWeight: 600}} onClick={() => navigateToProductDetails(product._id)}>Детали</button>
                                <button className='info' style={{fontWeight: 600}} onClick={() => setEditingProduct(product)}>Изменить</button>
                                <button className='info' style={{fontWeight: 600}} onClick={() => handleDeleteProduct(product._id)}>Удалит</button>
                            </div>
                            </div>
                        </div>
                    ))}
