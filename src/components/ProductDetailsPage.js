...
const ProductDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchProductDetails = async () => {
            try {
                setLoading(false);
                const productResponse = await axios.get(`http://localhost:5000/api/imported-products/${id}`);
                setProduct(productResponse.data);
        
                const clientsResponse = await axios.get(`http://localhost:5000/api/clients/product/${productResponse.data.id}`);
    ...
    return (
        <div style={{ marginLeft: `18%`, marginTop: `6%` }}>
            <Link to="/imported-products" style={{ padding: 5, marginLeft: 80, fontWeight: 600 }} className="info exit">Выход</Link>
            <div className="charts-container flex-space wrapper">
                ...
                <div style={{ marginLeft: 65 }} className="chart-item diogram-s">
                    ...
                    <div className="chart-legend">
                        ...
                        <div className="legend-item">
                            ...
                            <p>Доступно: <span style={{ color: COLORS[4], fontWeight: 500 }}>{availableLength}м</span></p>
                        </div>
                        <div className="legend-item">
                            <span className="color-box" style={{ backgroundColor: COLORS[1] }}></span>
                            <p>Продано: <span style={{ color: COLORS[4], fontWeight: 500 }}>{soldLength}м</span></p>
                        </div>
                    </div>
                </div>
                ...
                <div className="summary-container diogram-s" style={{ marginRight: `3%`, marginTop: 20 }}>
                    ...
                    <p>Общая сумма покупки: <span style={{ fontWeight: 600 }}>{calculateTotalClientPurchase()} c.</span></p>
                    <p>Сумма без выручки: <span style={{ fontWeight: 600 }}>{calculateTotalWithoutRevenue()} c.</span></p>
                    <p>Выручка: <span style={{ fontWeight: 600 }}>{calculateRevenue()} c.</span></p>
                    <p>Цена товара: <span style={{ fontWeight: 600 }}>{product.price} c.</span></p>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="client-list-container">
                    <div className='flex-space wrapper' style={{ marginTop: 15, marginLeft: `6%` }}>
                        <h3 style={{ marginLeft: -50, marginTop: -50, fontWeight: 600, fontSize: 22 }}>Клиенты, купившие этот продукт:</h3>
                    </div>
                    <div className="product-summary-a flex-space">
                        ...
                        <p className="details"><span></span></p>
                        <p className="details"><span></span></p>
                        <p className="details"><span>Куплено</span></p>
                        <p className="details"><span></span></p>
                            ...
                            {clients.map(client => (
                                <div key={client._id} className={`product-item`}>
                                    <div className="product-summary flex-space">                          
                                        <div key={client._id} style={{ borderRadius: 5, fontSize: 20, color: `#363d55`}} ><FontAwesomeIcon icon={faUser} /></div>
                                        <div style={{marginLeft: `-6%`}} className="details"><span>{client.name}</span></div>|
                                        <div className="details"><span>+{client.phone}</span></div>|
                                        <div className="details"><span>{client.purchasedLength}м</span></div>|
                                        <div className="details"><span>{client.purchasedPrice}c.</span></div>|
                                        <div className="details"><span>{client.purchasedLength * client.purchasedPrice}c.</span></div>|
                                    </div>
                                </div>
                            ))}
                        </ul>
