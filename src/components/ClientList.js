...
import AddClient from './AddClient'; // Убедитесь, что путь к файлу правильный
...
const ClientList = () => {
    ...
    const COLORS = ['#996DAA', '#FFBB28', '#c1c1c1', '#FF8042', '#000000'];  // Добавляем цвет для общей суммы покупок
        ...
        const fetchClientsAndDebts = async () => {            
            try {
                ...
                for (let client of clientsData) {
                    const productsResponse = await axios.get(`http://localhost:5000/api/clients/${client._id}/products`);
    ...
    const handleEditSubmit = async () => {
        try {
            await axios.put(`http://localhost:5000/api/clients/${editClientId}`, editClientData);
            setClients(clients.map(client => 
                client._id === editClientId ? { ...client, ...editClientData } : client
            ));
    ...
    .filter(client => 
        showOnlyDebtors ? clientDebts[client._id] > 0 : true  // Фильтрация по должникам
    );
    ...
    return (
        ...
        <Link to="/" style={{padding: 5}} className="info">Exit</Link> {/* Кнопка для возврата */}  
        ...
        <div style={{marginLeft: 10}} className="chart-legend">
            <div className="legend-item">
                ...
                <p>Общая сумма покупок: <span style={{ color: COLORS[4], fontWeight: 500 }}>{totalPurchases} c.</span></p>
            </div>
            <div className="legend-item">
                <span className="color-box" style={{ backgroundColor: COLORS[0] }}></span>
                <p>Оплаченная сумма: <span style={{ color: COLORS[4], fontWeight: 500 }}>{totalPaidAmount} c.</span></p>
            </div>
            <div className="legend-item">
                <span className="color-box" style={{ backgroundColor: COLORS[1] }}></span>
                <p>Общая сумма долгов: <span style={{ color: COLORS[4], fontWeight: 500 }}>{totalDebt} c.</span></p>
            </div>
        </div>
            </div>
        ...
        <hr/>
            ...
            {editClientId ? (
                <div className="edit-form">
                    ...
                    <input
                        ...
                        placeholder="Phone"
                    />
                    <button className="info" style={{fontWeight: 600}} onClick={handleEditSubmit}>Изменить</button>
                    <button className="info" style={{fontWeight: 600}} onClick={() => setEditClientId(null)}>Назад</button>
                </div>
            ) : null}
            ...
            <div className="product-summary-a flex-space">
                <p className="details"><span>Имя</span></p>
                <p className="details"><span></span></p>
                <p className="details"><span></span></p>
                <p className="details"><span></span></p>
                <p className="details"><span>Телефон</span></p>
                <p className="details"><span></span></p>
                <p className="details"><span></span></p>
                <p className="details"><span>Долг</span></p>
                <p className="details"><span></span></p>
                <p className="details"><span></span></p>
                <p className="details"><span></span></p>
                <p className="details"><span></span></p>
            ...
            {filteredClients.length > 0 ? (
                <ul style={{ marginTop: 15 }} className="client-list">
                    {filteredClients.map(client => (
                        <li key={client._id} style={{ borderRadius: 5, fontSize: 20, color: `#363d55`}} className="client-item flex-space">
                            <FontAwesomeIcon icon={faUser} />
                            <span className="details" style={{ paddingLeft: 5, marginLeft: `1.5%` }}>{client.name}</span>
                            <span className="details" style={{ paddingLeft: 5 }}>+{client.phone}</span>
                            <span style={{fontWeight: 500}} className="details">{clientDebts[client._id] ? `${clientDebts[client._id].toFixed(0)}c.` : 'Нет долгов'}</span>

                            <div className="client-actions">
                                <button style={{fontWeight: 600}} className="info" onClick={() => handleViewProducts(client._id)}>Продукты</button>
                                <button style={{fontWeight: 600}} className="info" onClick={() => handleEdit(client)}>Изменить</button>
                                <button style={{fontWeight: 600}} className="info" onClick={() => handleDelete(client._id)}>Удалит</button>
                            </div>
                        </li>
                    ))}
                </ul>
